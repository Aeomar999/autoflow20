...
| `SYSTEM` | `broadcastSystemNotification` in `src/features/notifications/server/system-notifier.ts`, driven by `npm run notify:system` | **live** (AF-M8-13). Maintenance, incident, and deprecation announcements, one row per workspace, written through `writeNotifications` like every other producer. A script rather than an in-app procedure because there is no platform-admin role - `Role` is org-scoped, so an operator procedure would mean inventing a cross-tenant privilege and exposing it on the public app. Dry-run by default; `--yes` sends, `--org` narrows. Idempotent per `(announcement, organization)` via dedupe key `system:<announcementId>:<orgId>`. |
