...
| S2 | High | Credentials encrypted with Cryptr (AES-CBC); **partially addressed 2026-08-24** (boot-time env validation + lazy key via `src/lib/env.ts`), **envelope module 2026-08-27 (AF-M3-01)**, **storage swap done 2026-08-27 (AF-M3-02)**, and **credential injection done 2026-08-27 (AF-M3-04)**: `Credential` now stores the sealed AEAD envelope (ciphertext/iv/authTag/wrappedDek/keyVersion, `20260827170000_credential_vault_af_m3_02`); engine resolves credentials via `resolveNodeCredentials` (single decrypt site), threads them into executors, and never merges them into context/output/traces; AI executors no longer call `openSecret` directly. Remaining: UI upgrade (AF-M3-03), in-use warnings, and `test` audit record. | `src/lib/encryption.ts` (legacy, converter only), `src/lib/crypto.ts`, `src/features/credentials/server/vault.ts`, `src/features/executions/server/credential-resolver.ts` |
...
| M3 | Credential vault + connectors | 🟠 In progress | AF-M3-01 ✅ (2026-08-27): envelope crypto module + `CREDENTIAL_MASTER_KEY` boot gate (16 unit tests). AF-M3-02 ✅ (2026-08-27): vault model + typed registry (8 kinds) + `create/update/remove/list/getOne/test` with **no plaintext read path** (42 new tests) + one-time legacy converter. AF-M3-04 ✅ (2026-08-27): credential injection into execution engine — `resolveNodeCredentials` as single decrypt site, AI executors consume `NodeRunParams.credentials`, secrets never in traces (13 new tests). Basic Cryptr code removed from the runtime path. No OAuth refresh, no connector framework. |
...
| Tests | **250 collected** — 238 always-on + 12 integration against real Postgres (visible skip without `TEST_DATABASE_URL`) | `npm test` |
