...
**Current milestone:** M3 credential vault + connectors — AF-M3-01/02/04 done; UI (M3-03), OAuth (M3-05), connectors (M3-06) next
...
| 5.2 | Visual builder & nodes | 🟠 | ~55% | Canvas saves via `workflows.saveGraph` with optimistic concurrency (revision); 9 node types registered in `src/nodes/` registry; autosave (1.5s debounce), save states (saved/saving/unsaved/failed), CONFLICT auto-reload, `beforeunload` guard. No palette-driven UI, no schema-driven config panel, no validation/linting. |
...
| Tests | **247 collected** — 235 always-on + 12 integration against real Postgres (visible skip without `TEST_DATABASE_URL`) | `npm test` |
