...
| M2 | Execution engine + traces | 🟠 In progress | AF-M2-01 ✅, AF-M2-02 ✅, AF-M2-03 ✅ (2026-08-26): expression context helpers — `buildTemplateContext` enriches executor context with `$json`, `$node["Name"]`, `$execution.id`, `$workflow.id`, `$now`. Engine tracks per-node outputs for `$node` resolution. `ExpressionError` exported for future use. `$env` and `$items` deferred. ADR-0007 amended; execution_engine.md §5 updated. 17 new tests (129 total). |
