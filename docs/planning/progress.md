...
**Current milestone:** M2 complete; next up — M3 credential vault + connectors
...
| S2 | High | Credentials encrypted with Cryptr (AES-CBC); **partially addressed 2026-08-24** (boot-time env validation + lazy key via `src/lib/env.ts`) and **AEAD envelope module landed 2026-08-27 (AF-M3-01)**: AES-256-GCM + per-record DEK + `CREDENTIAL_MASTER_KEY` KEK + `keyVersion` rotation, unit-tested; **storage swap of `Credential.value` to the envelope still pending AF-M3-02** | `src/lib/encryption.ts`, `src/lib/crypto.ts`, `src/lib/env.ts` |
...
| M3 | Credential vault + connectors | 🟠 In progress | AF-M3-01 ✅ (2026-08-27): envelope crypto module + `CREDENTIAL_MASTER_KEY` boot gate (16 unit tests). Basic CRUD + Cryptr remain until AF-M3-02; no OAuth refresh, no connector framework. |
...
| Tests | **195 collected** — 183 always-on + 12 integration against real Postgres (visible skip without `TEST_DATABASE_URL`) | `npm test` |
