...
| M2 | Execution engine + traces | 🟠 In progress | AF-M2-01 ✅, AF-M2-02 ✅, AF-M2-03 ✅, AF-M2-04 ✅ (2026-08-26): runner upgrades — branch-taken semantics (BFS reachability from trigger via taken edges; nodes with untaken incoming edges → SKIPPED), per-node timeout (default 60s, `Promise.race`), per-node retry with exponential backoff, `continueOnFail` (on failure mark outgoing edges taken + continue), concurrency keys (per-workflow limit 1, per-tenant limit 10). `_outputPort` convention for branching nodes. 137 tests. |
