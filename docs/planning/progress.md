...
**Current milestone:** M1 complete; next up — M2 execution engine enhancements
...
| M2 | Execution engine + traces | 🟠 In progress | AF-M2-01 ✅, AF-M2-02 ✅, AF-M2-03 ✅, AF-M2-04 ✅, AF-M2-05 ✅, AF-M2-06 ✅, AF-M2-07 ✅, AF-M2-08 ✅ (2026-08-27): In-editor test runs — `workflows.testRun` runs the current draft via inline `graphSnapshot` (engine never reads persisted workflow for test runs); Test workflow button (whole draft) + Test node button (single selected node, skips all predecessors via `skipNodes`, stops after target via `endAfterNodeId`); engine now consumes `skipNodes` (also fixing retry-from-node); run recorded `mode: TEST`, `executions.list` excludes TEST by default with an explicit mode filter; expandable per-node results delivered on the execution detail page (realtime status painting + inline output/error viewers), TEST badge on detail header. 8 new unit tests for the test-run planner (linear/trigger-target/diamond-isolation/missing-target/invalid-draft). 167 tests, tsc + lint clean. |
