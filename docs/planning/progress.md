...
**Current milestone:** M3 credential vault + connectors — AF-M3-01/02 done; UI (M3-03) and injection (M3-04) next
...
credential vault with a type registry and no plaintext read path, two webhook triggers plus manual trigger, and execution
...
| 5.4 | Governance, security, compliance | 🔴 | ~18% | Better Auth (email/password + GitHub/Google OAuth) + Polar premium gate + envelope-encrypted credential vault (AF-M3-01/02: AES-256-GCM per-record DEK, typed registry, no plaintext read path, `.strict()` public schema + tests). No RBAC/orgs/audit/SSO/quotas; webhook auth gaps (§5 S1). |
...
| S2 | High | Credentials encrypted with Cryptr (AES-CBC); **partially addressed 2026-08-24** (boot-time env validation + lazy key via `src/lib/env.ts`), **envelope module 2026-08-27 (AF-M3-01)**, and **storage swap done 2026-08-27 (AF-M3-02)**: `Credential` now stores the sealed AEAD envelope (ciphertext/iv/authTag/wrappedDek/keyVersion, `20260827170000_credential_vault_af_m3_02`); legacy rows converted by `npm run migrate:credentials`. Remaining: enforce in-use warnings + node-requirement filtering (AF-M3-03/04) and add a `test` audit record. | `src/lib/encryption.ts` (legacy, converter only), `src/lib/crypto.ts`, `src/features/credentials/server/vault.ts` |
...
| Tests | **237 collected** — 225 always-on + 12 integration against real Postgres (visible skip without `TEST_DATABASE_URL`) | `npm test` |
