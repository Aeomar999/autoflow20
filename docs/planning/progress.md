...
| M2 | Execution engine + traces | 🟠 In progress | AF-M2-01 ✅, AF-M2-02 ✅, AF-M2-03 ✅, AF-M2-04 ✅, AF-M2-05 ✅, AF-M2-06 ✅ (2026-08-26): Executions API — `executions.list` (filters + pagination, excludes large IO columns), `getOne` (full run + ordered traces), `cancel`/`retry`/`retryFromNode` (BFS upstream-skip via `computeSkipNodes`); `workflows.run` pre-creates the execution record then emits the event; `sendWorkflowExecution` returns `{ eventId }`; engine honors pre-created executions via `event.data.executionId`. 159 tests, tsc + lint clean. |
