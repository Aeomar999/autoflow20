...
- [x] **AF-M5-03** `ai.extract` node: structured extraction to a user-defined schema · 2d — **done**: `src/nodes/ai/extract/` — `definition.ts` (isomorphic `configSchema` with `model`, provider credential selects, `content` prompt template, `fields` array of `{ name, type, description }` plus `jsonSchema` escape hatch string, `variableName`), `execute.ts` (server-only; `createLanguageModel` via registry, `buildExtractionSchema` dynamically building JSON Schema with required properties and strict typing, `generateObject` with AI SDK `jsonSchema()`, `step.ai.wrap` tracing, result saved to `data.variableName` as `{ data, model }`), registered as `ai.extract` in manifest + registry; editor config schema extended with `fieldList` support in `config-schema.ts` and `node-config-panel.tsx` with DOM tests; 29 unit tests covering extraction schema builder, JSON schema override, credential routing, template compilation, and error handling. Full suite 524 passed | 17 skipped (59 files); tsc, biome, and build clean.
