...
- ⬜ **AF-M8-11** Convert `Execution.costUsd` / `NodeExecution.costUsd` / `AiResponseCache.costUsd` from `Float` to `Decimal(12,6)` · 1d · *(added 2026-08-31, found during AF-M5-08)* — `docs/architecture/data_model.md` §2.4 has always specified `Decimal(12,6)`, "never a float"; the M2 migration shipped `Float` and AF-M5-05/07/08 built capture, caching, and reporting on it. Every read rounds to micro-dollars, which contains but does not remove the drift. Expand-migrate-contract: add the `Decimal` columns, backfill, cut reads over, drop the floats.
