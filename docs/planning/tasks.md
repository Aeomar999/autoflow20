...
- ⬜ **AF-M8-13** Fix flaky `src/features/templates/server/instantiate.test.ts:116` template-instantiation test · 0.25d · *(added 2026-08-31, found during AF-M8-02 verification)* — the `not.toContain("$node.a")` assertion fails nondeterministically whenever a freshly-rotated cuid happens to start with the letter `a` (the rotated id `$node.ax...` contains `$node.a`). Assert on the mapped value (compare against `idMap`), not a prefix that collides with the generated id's first letter. AF-M7-01 code; passes in isolation.
