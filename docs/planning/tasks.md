...
**The finding.** The 1000-step ceiling is not the constraint. The engine opens 4 steps per node, so step count allows ~248 nodes — but **18 of 21 executors return `{ ...context, … }`**, and Inngest memoises every step return, so run state grows as `b × n(n+1)/2` against a **32 MB** cap while each return must also fit under **4 MiB**. At 100 KB of output per node — one ordinary HTTP or AI node — the engine tops out near **24 nodes**, a tenth of what the step ceiling implies. Nothing bounds node output; the only truncation in the engine is `MAX_STACK_LENGTH` for stack traces. Cancellation is also **not wired**: `POST /api/v1/executions/:id/cancel` marks the row cancelled and the Inngest run continues to completion.
