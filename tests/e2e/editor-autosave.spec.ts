...
test("place 3 nodes, connect them, configure one, hard-refresh, everything is exactly as left", async ({
  ...
  await page.waitForURL(
    (url) => url.pathname === "/" || url.pathname === "/workflows",
  );
  ...
  await page
    .getByRole("button", { name: /create workflow|new workflow/i })
    .click();
  ...
  for (let i = 0; i < 3; i++) {
    ...
    await page
      .getByRole("button", { name: /HTTP Request/i, exact: false })
      .click();
  ...
  async function connectNodes(sourceLoc: Locator, targetLoc: Locator) {
    const sourceHandle = sourceLoc
      .locator(".react-flow__handle-source")
      .first();
    const targetHandle = targetLoc
  ...
  await expect(page.getByLabel(/endpoint/i)).toHaveValue(
