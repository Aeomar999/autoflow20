...
export const execute: NodeRun<OffboardingChecklistData> = async ({
  data,
  context,
  resolve,
  step,
}) => {
  const where = "Offboarding Checklist node";
  ...
  const checklist = await step.run("build-offboarding-checklist", async () => ({
    phase: "OFFBOARDING",
    role,
