...
import { createEstimate } from "@/features/quickbooks/server/entities";
...
export const execute: NodeRun<SalesDocumentData> = async ({
  ...
  step.run("qbo-create-estimate", async () => {
    if (!data.variableName) {
      throw new NonRetriableError(
        "QuickBooks Create Estimate node: Variable name not configured",
      );
    }
    if (!data.customerId) {
      throw new NonRetriableError(
        "QuickBooks Create Estimate node: Customer ID not configured",
      );
    }
    if (!data.lines) {
      throw new NonRetriableError(
        "QuickBooks Create Estimate node: Lines not configured",
      );
    }

    const where = "QuickBooks Create Estimate node";
    ...
    const document = await createEstimate(connection, {
      customerId,
